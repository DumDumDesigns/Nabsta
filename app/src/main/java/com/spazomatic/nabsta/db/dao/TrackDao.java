...
public class TrackDao extends AbstractDao<Track, Long> {
    ...
    */
    public static class Properties {
        ...
        public final static Property Song_id_fk = new Property(3, Long.class, "song_id_fk", false, "SONG_ID_FK");
        public final static Property Artist_id_fk = new Property(4, Long.class, "artist_id_fk", false, "ARTIST_ID_FK");
        public final static Property Image_id_fk = new Property(5, Long.class, "image_id_fk", false, "IMAGE_ID_FK");
    };

    private DaoSession daoSession;

    private Query<Track> artist_TracksQuery;
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        ...
        db.execSQL("CREATE TABLE " + constraint + "'TRACK' (" + //
                ...
                "'SONG_ID_FK' INTEGER," + // 3: song_id_fk
                "'ARTIST_ID_FK' INTEGER," + // 4: artist_id_fk
                "'IMAGE_ID_FK' INTEGER);"); // 5: image_id_fk
    }
    ...
    public Track readEntity(Cursor cursor, int offset) {
        Track entity = new Track( //
            ...
            cursor.isNull(offset + 3) ? null : cursor.getLong(offset + 3), // song_id_fk
            cursor.isNull(offset + 4) ? null : cursor.getLong(offset + 4), // artist_id_fk
            cursor.isNull(offset + 5) ? null : cursor.getLong(offset + 5) // image_id_fk
        );
        return entity;
    }
    ...
    public void readEntity(Cursor cursor, Track entity, int offset) {
        ...
        entity.setSong_id_fk(cursor.isNull(offset + 3) ? null : cursor.getLong(offset + 3));
        entity.setArtist_id_fk(cursor.isNull(offset + 4) ? null : cursor.getLong(offset + 4));
        entity.setImage_id_fk(cursor.isNull(offset + 5) ? null : cursor.getLong(offset + 5));
     }
    ...
    /** Internal query to resolve the "tracks" to-many relationship of Artist. */
    public List<Track> _queryArtist_Tracks(Long artist_id_fk) {
        synchronized (this) {
            if (artist_TracksQuery == null) {
                QueryBuilder<Track> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.Artist_id_fk.eq(null));
                artist_TracksQuery = queryBuilder.build();
            }
        }
        Query<Track> query = artist_TracksQuery.forCurrentThread();
        query.setParameter(0, artist_id_fk);
        return query.list();
    }

    /** Internal query to resolve the "tracks" to-many relationship of Song. */
    public List<Track> _querySong_Tracks(Long song_id_fk) {
        synchronized (this) {
            if (song_TracksQuery == null) {
                QueryBuilder<Track> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.Song_id_fk.eq(null));
                song_TracksQuery = queryBuilder.build();
            }
        }
        Query<Track> query = song_TracksQuery.forCurrentThread();
        query.setParameter(0, song_id_fk);
        return query.list();
    }
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(" LEFT JOIN IMAGE T1 ON T.'IMAGE_ID_FK'=T1.'_id'");
            builder.append(' ');
